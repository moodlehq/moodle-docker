...
IF "%MOODLE_DOCKER_DB%"=="pgsql" (
    IF "%MOODLE_DOCKER_DB_VERSION%"=="" (
        SET MOODLE_DOCKER_DB_VERSION=12
    )
) ELSE IF "%MOODLE_DOCKER_DB%"=="mysql" (
    IF "%MOODLE_DOCKER_DB_VERSION%"=="" (
        SET MOODLE_DOCKER_DB_VERSION=5.7
    )
) ELSE IF "%MOODLE_DOCKER_DB%"=="mariadb" (
    IF "%MOODLE_DOCKER_DB_VERSION%"=="" (
        SET MOODLE_DOCKER_DB_VERSION=10.7
    )
)
...
SET filename=%BASEDIR%\db.%MOODLE_DOCKER_DB%-%MOODLE_DOCKER_DB_VERSION%.yml
if exist %filename% (
